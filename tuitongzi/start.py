...
def compareToBanker(data, request, settle, b):
    ...
    banker_multiple = Tuitongzi.get_multiple(banker_type, data.gameRules)
    ...
    for u in request.userSettleData:
        if u.userId != request.banker:
            ...
            user_multiple = Tuitongzi.get_multiple(user_type, data.gameRules)
            ...
            if user_type < banker_type or (user_type == banker_type and user_value < banker_value):
                ...
                # TODO
                if MahjongUtils.get_card_value(banker_array_cards[1]) < MahjongUtils.get_card_value(
                        user_array_cards[1]):
                    userSettleResult.win = user_multiple * u.score
                    win -= user_multiple * u.score
                elif MahjongUtils.get_card_value(banker_array_cards[1]) > MahjongUtils.get_card_value(
                        user_array_cards[1]):
                    userSettleResult.win = -banker_multiple * u.score
                    win += banker_multiple * u.score

                # if banker_array_cards[1] != 31 and (
                #         MahjongUtils.get_card_value(banker_array_cards[1]) < MahjongUtils.get_card_value(
                #         user_array_cards[1]) or user_array_cards[1] == 31):
                #     userSettleResult.win = user_multiple * u.score
                #     win -= user_multiple * u.score
                # else:
                #     userSettleResult.win = -banker_multiple * u.score
